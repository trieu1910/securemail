    ...
    <div className="my-6 flex flex-col items-center gap-0 sm:flex-row sm:gap-0 sm:overflow-x-auto">
      {steps.map((s, i) => (
        <div key={s.label} className="flex items-center">
          <div className="flex flex-col items-center rounded-lg border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-3 shadow-sm">
...
{"version":"1.0","mode":"password","ciphertext":"...","iv":"...","encryptedKey":"...","salt":"..."}`}</Code>
      ...
      <div className="my-4 space-y-3">
        {[
          ...
          <div key={item.title} className="flex gap-3 rounded-lg border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
...
export function Docs() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900 font-[Lexend] text-slate-800 dark:text-gray-200 antialiased">
      {/* ─── Top nav ─── */}
      <nav className="sticky top-0 z-50 border-b border-slate-200 dark:border-gray-700 bg-white/90 dark:bg-gray-900/90 backdrop-blur">
        <div className="mx-auto flex max-w-[90rem] items-center justify-between px-6 py-3">
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => navigate('/')} className="flex cursor-pointer items-center gap-2">
              ...
              <span className="text-base font-semibold text-slate-900 dark:text-gray-100">SecureMail</span>
            </button>

            <span className="rounded-full bg-slate-100 dark:bg-gray-700 px-3 py-1 text-xs font-semibold text-slate-500 dark:text-gray-400">{t('Docs', 'Tài liệu')}</span>
          </div>
      ...
      <div className="mx-auto flex max-w-[90rem]">
        ...
        <aside className={`fixed inset-y-0 left-0 z-40 w-64 sm:w-72 transform overflow-y-auto border-r border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 pt-16 transition-transform lg:sticky lg:top-14 lg:block lg:h-[calc(100vh-3.5rem)] lg:translate-x-0 ${
          sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        }`}>

          <nav className="relative z-10 bg-white dark:bg-gray-800 p-6">
            {nav.map((group) => (
              <div key={group.group} className="mb-6">
                <p className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-gray-500">{group.group}</p>
                      ...
                      <button
                        ...
                        className={`w-full cursor-pointer rounded-lg px-3 py-2 text-left text-sm transition-colors ${
                          activeId === item.id
                            ...
                            : 'text-slate-600 dark:text-gray-400 hover:bg-slate-50 dark:hover:bg-gray-700 hover:text-slate-900 dark:hover:text-gray-200'
                        }`}
                      >
                        {item.label}
                      </button>
